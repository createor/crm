...
-- 资产表
create table if not exists crm.crm_manage (
...
-- 资产表头部
create table if not exists crm.crm_header (
...
-- 下拉选项取值表
create table if not exists crm.crm_options (
    `id` int auto_increment primary key comment "id",
...
-- 资产表图表展示
create table if not exists crm.crm_echart (
    `id` int auto_increment primary key comment "id",
...
-- 白名单
create table if not exists crm.crm_white_list (
    `id` int auto_increment primary key comment "id",
...
-- 日志表
create table if not exists crm.crm_log (
...
-- 系统配置表
create table if not exists crm.crm_setting (
    `type` varchar(40) not null primary key comment "配置类型",
    `value` tinyint(1) default 0 comment "值:0-关闭,1-开启",
    `desc` varchar(255) comment "注释"
) engine=InnoDB default charset=utf8mb4;
-- 文件表
create table if not exists crm.crm_file (
...
create table if not exists crm.crm_task (
    ...
    `create_user` varchar(100) comment "创建者",
    `create_time` datetime default current_timestamp comment "创建时间",
    unique key `task_id_idx` (`id`)
) engine=InnoDB default charset=utf8mb4;
-- 任务结果表
create table if not exists crm.crm_detect_result (
    `id` int auto_increment primary key comment "id",
...
-- 过期通知任务表
create table if not exists crm.crm_notify (
    `id` varchar(40) not null unique primary key comment "任务id",
    `name` varchar(255) not null comment "任务名称",
    ...
    `update_user` varchar(100) comment "更新者",
    `update_time` datetime default current_timestamp on update current_timestamp comment "更新时间",
    unique key `notify_id_idx` (`id`)
) engine=InnoDB default charset=utf8mb4;
-- 用户通知表
create table if not exists crm.crm_notice (
    `id` int auto_increment primary key comment "id",
...
-- 历史记录表
create table if not exists crm.crm_history (
    ...
    `create_user` varchar(100) comment "创建者",
    `create_time` datetime default current_timestamp comment "创建时间",
    unique key `history_id_idx` (`id`)
) engine=InnoDB default charset=utf8mb4;
-- 初始化数据表
-- 初始化管理员用户
insert into crm.crm_user (`name`, `username`, `password`, `pwd_expire_time`) select "管理员", "admin", "07967d4a89a0172d1f20c53424d1860e", "2099-12-31" where not exists (select 1 from crm.crm_user where `username` = "admin");
-- 初始化系统配置
insert into crm.crm_setting (`type`, `value`, `desc`) select "enable_failed", 1, "是否开启失败锁定,1-开启,0-关闭" where not exists (select 1 from crm.crm_setting where `type` = "enable_failed");
insert into crm.crm_setting (`type`, `value`, `desc`) select "enable_white", 0, "是否开启白名单模式,1-开启,0-关闭" where not exists (select 1 from crm.crm_setting where `type` = "enable_white");
insert into crm.crm_setting (`type`, `value`, `desc`) select "enable_single", 1, "是否开启单点登录功能,1-开启,0-关闭" where not exists (select 1 from crm.crm_setting where `type` = "enable_single");
insert into crm.crm_setting (`type`, `value`, `desc`) select "failed_count", 3, "登陆失败次数" where not exists (select 1 from crm.crm_setting where `type` = "failed_count");
insert into crm.crm_setting (`type`, `value`, `desc`) select "enable_watermark", 1, "是否开启水印,1-开启,0-关闭" where not exists (select 1 from crm.crm_setting where `type` = "enable_watermark");
-- 初始化背景图片文件
insert into crm.crm_file (`uuid`, `filename`, `filepath`, `affix`) select "crm", "crm.png", 2, "png" where not exists (select 1 from crm.crm_file where `uuid` = "crm");
insert into crm.crm_file (`uuid`, `filename`, `filepath`, `affix`) select "default", "default.png", 2, "png" where not exists (select 1 from crm.crm_file where `uuid` = "default");
insert into crm.crm_file (`uuid`, `filename`, `filepath`, `affix`) select "nodata", "nodata.png", 2, "png" where not exists (select 1 from crm.crm_file where `uuid` = "nodata");
-- 初始化白名单
insert into crm.crm_white_list (`ip`, `description`) select "127.0.0.1", "本地地址" where not exists (select 1 from crm.crm_white_list where `ip` = "127.0.0.1");
