...
from app.src.models import db_session, User, Log, Notice, Notify
...
def userLogin():
    ...
    username = reqData["username"]  # 获取用户名
    password = reqData["password"]  # 获取密码
    captcha = reqData["captcha"]    # 获取验证码

    if not all([username, password, captcha]):  # 校验参数是否有值
        return jsonify({"code": -1, "message": "请求参数不完整"}), 400

    cache_captcha = redisClient.getData(captcha_id)  # 获取redis中记录的验证码
    ...
    if result.password != hashlib.md5((password.upper() + SALE).encode()).hexdigest().lower():  # 密码错误
        if bool(int(redisClient.getData("crm:system:enable_failed"))):  # 判断是否开启锁定功能
            if redisClient.getData(f"crm:{username}:failed") and int(redisClient.getData(f"crm:{username}:failed")) > int(redisClient.getData("crm:system:failed_count")):  # 判断是否大于设置错误次数
                try:  # 更新user表
...
@verify(allow_methods=["GET"], module_name="查询用户", is_admin=True, check_ip=True)
def queryUser():
    '''查询用户'''
    ...
    try:
        result = db_session.query(User).filter(User.username != "admin").order_by(User.uid.asc()).offset((page - 1) * limit).limit(limit).all()
    finally:
        db_session.close()
...
@verify(allow_methods=["POST"], module_name="编辑用户", is_admin=True, check_ip=True)
def editUser():
    '''编辑用户'''
    reqData = request.get_json()  # 获取请求数据
...
@verify(allow_methods=methods.ALL, module_name="锁定用户", is_admin=True, check_ip=True)
def lockUser():
    '''锁定用户'''
    reqData = request.get_json()  # 获取请求数据
...
@verify(allow_methods=["POST"], module_name="解锁用户", is_admin=True, check_ip=True)
def unlockUser():
    '''解锁用户'''
    reqData = request.get_json()  # 获取请求数据
...
def getMail():
    ...
    page = int(args.get("page", 1))   # 获取页码
    limit = int(args.get("limit", 5)) # 获取每页数量

    try:  # 查询用户创建并开启状态的过期通知
        user_notify = db_session.query(Notify.id).filter(Notify.create_user == g.username, Notify.status == 1).all()
    finally:
        db_session.close()
    ...
    try:
        count = db_session.query(Notice).filter(Notice.is_read == 0, Notice.notify_id.in_([u.id for u in user_notify])).count()
    finally:
        db_session.close()
    ...
    try:
        email = db_session.query(Notice).filter(Notice.is_read == 0, Notice.notify_id.in_([u.id for u in user_notify])).order_by(Notice.create_time.desc()).offset((page - 1) * limit).limit(limit).all()
    finally:
        db_session.close()
...
def readMail():
    ...
    msg_id = args.get("id", None)  # 获取通知id
