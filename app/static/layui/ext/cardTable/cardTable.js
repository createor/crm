layui.define(['table', 'laypage','jquery', 'element','laytpl'], function(exports) {
	...
	function initData(tempData, option) {
		...
		if (option.response.countName.split(".").length > 1) {
			data.count = tempData[option.response.countName.split(".")[0]][option.response.countName.split(".")[1]]
		} else {
			data.count = tempData[option.response.countName];
		}
		if (option.response.dataName.split(".").length > 1) {
			var dataList = tempData[option.response.dataName.split(".")[0]][option.response.dataName.split(".")[1]];
		} else {
